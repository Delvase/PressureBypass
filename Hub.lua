...
local adornBoxes = {} -- BoxHandleAdornment: works even when character is transparent (Ghost)
...
-- Reliable anti-Ghost: every frame set LocalTransparencyModifier = 0 (client-only).
-- Roblox Highlight does not render on transparent parts (engine limit).
-- BoxHandleAdornment on HRP still draws through walls/invis.
local function stripLocalInvis(char)
...
local function ensureAdornBox(plr, root, color)
    local box = adornBoxes[plr]
    if not box or not box.Parent then
        if box then pcall(function() box:Destroy() end) end
        box = Instance.new("BoxHandleAdornment")
        box.Name = "HubAdorn_" .. plr.Name
        box.AlwaysOnTop = true
        box.ZIndex = 10
        box.Transparency = 0.45
        box.Size = Vector3.new(2.2, 5.2, 2.2)
        box.Adornee = root
        box.Parent = espGuiParent
        adornBoxes[plr] = box
    end
    box.Adornee = root
    box.Color3 = color
...
local function hideAdornBox(plr)
    local box = adornBoxes[plr]
    if box then
        box.Visible = false
    end
...
local function destroyAdornBox(plr)
    local box = adornBoxes[plr]
    if box then
...
-- PreRender: force local visibility AFTER game applies Ghost transparency
RunService:BindToRenderStep("HubAntiGhost", Enum.RenderPriority.Camera.Value + 1, function()
    if not ESPMaster then return end
    for _, plr in ipairs(allPlayersCache) do
        if plr ~= LocalPlayer then
            local role = getRole(plr)
            if not isDead(plr) and (shouldShow("Chams", role) or shouldShow("ESP", role) or shouldShow("Tracers", role)) then
                local char = plr.Character
                if char then
                    stripLocalInvis(char)
                end
            end
        end
    end
