...
local function getEspRoot(plr)
    ...
    if char then
        ...
        if hrp then
            lastKnownRoot[plr] = hrp.Position
            return hrp, char
        end
        local head = char:FindFirstChild("Head")
        if head then
            lastKnownRoot[plr] = head.Position
            return head, char
        end
        local part = char:FindFirstChildWhichIsA("BasePart")
        if part then
            lastKnownRoot[plr] = part.Position
            return part, char
        end
    end
    return nil, char
...
local function updateESP()
    ...
    for _, plr in ipairs(allPlayersCache) do
        if plr ~= LocalPlayer then
            ...
            -- Keep ESP for invisible murderer: role still Murderer and not Dead on server
            if not root then
                if role == "Murderer" and not dead and lastKnownRoot[plr] then
                    local pos = lastKnownRoot[plr]
                    local screenPos, onScreen = Camera:WorldToViewportPoint(pos)
                    local dist = myHRP and (myHRP.Position - pos).Magnitude or 0
                    if dist < 0.001 then dist = 0.001 end
                    local color = getDisplayColor(role)
