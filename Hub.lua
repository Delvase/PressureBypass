    ...
    for parent, byName in pairs(groups) do
        for name, info in pairs(byName) do
            if info.count >= 5 then
                ...
                if nameHasHint(name, PICKUP_NAME_HINTS) then score = score + 2 end
                if nameHasHint(parent.Name, PARENT_NAME_HINTS) then score = score + 2 end
                if sample:IsA("MeshPart") or sample:FindFirstChildOfClass("SpecialMesh") then
                    score = score + 1
                end
                if info.count <= 80 then score = score + 1 end
...
local function scanBagFromInterface()
    ...
    for _, object in ipairs(playerGui:GetDescendants()) do
        ...
        if current and object.Visible then
            ...
            local score = 0
            if string.find(context, "coin", 1, true) then score = score + 4 end
            if string.find(context, "bag", 1, true) then score = score + 3 end
            if string.find(context, "candy", 1, true) then score = score + 2 end
            if string.find(context, "currency", 1, true) then score = score + 1 end
            if object.Name == "Coins" then score = score + 4 end
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    retargetAccumulator = retargetAccumulator + deltaTime
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not farmingActively then
        if vel.Magnitude < 90 and pos.Y > voidThreshold then
            safePosAccumulator = safePosAccumulator + deltaTime
