...
task.spawn(function()
    while task.wait(2) do
        if ESPMaster or (State and State.isFarming) then
            refreshRoles()
        end
    end
end)
...
-- ==================== AUTO FARM (autofarm_mm2 + роли с сервера) ====================
...
local function getCurrentCoins()
    ...
    if not ok or res == nil then return 0 end
    if type(res) == "number" then return res end
    local digits = tostring(res):match("%d+")
...
-- Avoid Murder: только при включённом Auto Farm + роли с сервера
local function hasNearbyMurderer()
    if not State.isFarming or not Settings.AutoFarmEnabled then
        return false
    end
    if not Settings.AvoidMurder then
        return false
    end
    ...
    local function isMurdererPlayer(plr)
        if not plr or plr == LocalPlayer then return false end
        local entry = roleTable and roleTable[plr.Name]
        if entry and tostring(entry.Role) == "Murderer" and entry.Dead ~= true then
            return true
        end
        if currentMurderer and plr == currentMurderer then
            if not entry or entry.Dead ~= true then
                return true
            end
        end
        return false
    ...
    if currentMurderer and isMurdererPlayer(currentMurderer) and currentMurderer.Character then
        local mHRP = currentMurderer.Character:FindFirstChild("HumanoidRootPart")
        if mHRP and (mHRP.Position - hrp.Position).Magnitude <= 30 then
            return true
        end
    end

    for _, player in pairs(Players:GetPlayers()) do
        if isMurdererPlayer(player) and player.Character then
            local otherHRP = player.Character:FindFirstChild("HumanoidRootPart")
            if otherHRP and (otherHRP.Position - hrp.Position).Magnitude <= 30 then
                return true
            end
        end
    end
    ...
    task.spawn(function()
        while State.isFarming do
            ...
            local success = pcall(function()
                ...
                -- Auto Reset только при активном фарма
                if State.isFarming and Settings.AutoFarmEnabled and Settings.AutoReset then
                    local coins = getCurrentCoins()
                    local bagFull = isBagFull()
                    if bagFull or coins >= Settings.CoinLimit then
                        State.isActivelyFlying = false
                        State.currentTargetCoin = nil
                        removePhysics()
                        pcall(function()
                ...
                local reachedTarget = flyToPoint(targetPos, targetCoin, hrp, torso, lockedRotation)
