...
local function createGunESP(drop)
    ...
    label.TextSize = 14
    label.TextColor3 = espSettings.GunColor
    label.Text = "[GUN]"
            ...
            if not part then
                ...
                if not (showBox or showName or showTracer or showChams or showOutline) then
                    ...
                    if showName and obj.billboard then
                        ...
                        obj.billboard.Enabled = true
                        obj.label.Text = "[GUN]"
                        obj.label.TextColor3 = espSettings.GunColor
                        obj.distLabel.Text = string.format("[%d studs]", math.floor(dist))
                    elseif obj.billboard then
                        obj.billboard.Enabled = false
                    end
...
EspTab:CreateColorPicker({
    ...
    Flag = "ESPGunColor",
    Callback = function(c) espSettings.GunColor = c end
})
...
EspTab:CreateButton({
    ...
    Callback = function()
        refreshRoles()
        notify("ESP", "Роли обновлены с сервера")
    end
})
