...
local function getPlayerStatus()
    ...
    if entry then
        ...
        if role == "Murderer" or role == "Sheriff" or role == "Innocent" or role == "Hero" then
            return "Match"
        end
    end
    return "Spectate"
...
local function GrabGun()
    ...
    if PlayerStatus ~= "Match" then
        if SuccessNotify then
            notify("GrabGun", "Skipped (Spectate)")
        end
        return
    end
...
local function refresh_target()
    ...
    if m and type(m.PlayerData) == "table" then
        ...
        local entry = roleTable and roleTable[LocalPlayer.Name]
        if entry then
            SA.am_sheriff = not entry.Dead and (entry.Role == "Sheriff" or entry.Role == "Hero")
        else
            local char = LocalPlayer.Character
...
local function install_hooks()
    ...
    if not hook_mouse then
        hook_mouse = function(self, ...)
            ...
            if orig_mouse then return orig_mouse(self, ...) end
        end
        ...
        hook_screen = function(self, x, y, ...)
            ...
            local ok, cf = pcall(resolve_shot)
            if ok and cf then return compensate_resolve(cf) end
            if orig_screen then return orig_screen(self, x, y, ...) end
        end
    end
