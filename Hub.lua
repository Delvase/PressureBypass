...
-- ===== Silent Aim (exact logic from silent_aim_extracted) =====
...
local S = {
    enabled    = false,   -- силент включён
    am_sheriff = false,   -- локальный игрок шериф / герой
    predict    = true,    -- предикция
    force      = false,   -- force-mode (антиаим обход через аттачмент)
    auto_on    = false,   -- авто-стрельба
    auto_delay = 0.25,    -- задержка между авто-выстрелами
    fire_gap   = 0.15,    -- минимальный интервал
    stand_off  = 0x1E,    -- 30 stud — откат origin при force
    last_shot  = 0,
}
...
local P = {
    lead_scale  = 1.0,
    lead_add    = 0,
    ring        = 0x20,   -- 32 — размер скользящего окна скорости
    acc_max     = 0x78,   -- 120 stud/s² — ограничение ускорения
    hit_r       = 0x5,    -- 5 stud — радиус попадания для облака хитов
    pad         = 0x2,    -- 2 stud — отступ коридора
    min_span    = 0x8,    -- 8 stud — минимальная длина коридора
    max_span    = 0x3C,   -- 60 stud — максимум
}
...
local _SNAP_CAP = 0x40 -- 64
local snap_t    = table.create(_SNAP_CAP, 0)
local snap_p    = table.create(_SNAP_CAP, Vector3.zero)
...
local MAX_RANGE = 0x3E8  -- 1000 stud — максимальная дальность луча
...
local function trace(origin, delta)
    ...
    return _ws:Raycast(origin, delta, _ray_params)
end
...
local _ping_samples = {}
local _PING_CAP = 0xC  -- 12 samples
...
local function lead_time()
    return ping_avg() * P.lead_scale + P.lead_add
end
...
local function track(now)
    ...
    if snap_n >= 3 then
        local t0, p0 = snap_get(0)
        local tn, pn = snap_get(math.min(snap_n - 1, 0xF))
        local dt = t0 - tn
        if dt > 0.05 then
            TR.vel   = (p0 - pn) / dt
            TR.ready = true
...
local function predict_from(base, vel_y, dt, vel_h, _now)
    local g     = grav()
...
local function lead_offset()
    ...
    local pt   = predict_from(base, 0, lead_time(), fh, now)
    local off  = pt - base
    pred_stamp = now
    pred_off   = off
    return pred_off
...
local function gun_attachment()
    ...
    local handle = gun:FindFirstChild("Handle")
        or gun:FindFirstChildWhichIsA("BasePart")
...
local function shot_shift(dt)
    ...
    if TR.air then
        ...
        if TR.jumping and TR.jump_v > 0 and g > 0
            and phase <= TR.jump_v / g and modeled > vy
        then
            vy = modeled
        end
        shift = Vector3.new(
            shift.X,
            vy * dt - g * lt * dt - 0.5 * g * dt * dt,
            shift.Z
        )
    end
    return shift
...
local function resolve_force()
    ...
    local dir
    if vel and vel.Magnitude > 0x3 then  -- > 3 stud/s
        dir = vel.Unit
    else
        local mine = origin_cframe()
        if mine then
            local delta = live - mine.Position
            if delta.Magnitude > 0x2 then dir = delta.Unit end
        end
        if not dir then dir = Vector3.new(0, -1, 0) end
    end

    local front_dist = math.max(P.min_span, vel.Magnitude * dt + 0x8)
    local back  = live - dir * 0x6          -- 6 stud за целью
    ...
    if hit and hit.Instance then
        ...
        if not (char and (inst == char or inst:IsDescendantOf(char))) then
            back = live - dir * 0x2  -- подтягиваем если стена
        end
    end
    ...
    while want > 0 do
        ...
        if (front - probe).Magnitude <= P.max_span
            and los_clear(probe, live)
        then
            back = probe
            break
        end
    ...
    if (front - back).Magnitude > P.max_span then
        back = front - u * P.max_span
    end
...
local function compensate_force(ocf, acf, started)
    ...
    return CFrame.new(ocf.Position + shift, acf.Position + shift),
           CFrame.new(acf.Position + shift)
end

local function resolve_shot()
    if not S.enabled or not S.am_sheriff or not target_alive() then
        return nil
    end
...
local function install_hooks()
    ...
    if not hook_mouse then
        hook_mouse = function(self, ...)
            ...
            return orig_mouse(self, ...)  -- фоллбэк на оригинал
        end
    ...
    if type(m.GetMouseTargetCFrame) == "function"
        and m.GetMouseTargetCFrame ~= hook_mouse
    then
        orig_mouse = m.GetMouseTargetCFrame
        pcall(function() m.GetMouseTargetCFrame = hook_mouse end)
    end

    if type(m.GetTargetPosition) == "function"
        and m.GetTargetPosition ~= hook_screen
    then
...
local gap_samples    = {}
local _GAP_CAP       = 0x8  -- 8

local function gap_reset()
    table.clear(gap_samples)
end
...
local function fire_gap_avg()
    local n = #gap_samples
    if n == 0 then return S.fire_gap end
    local s = 0
...
local function auto_step(now)
    if not S.auto_on or not S.enabled or not S.am_sheriff
        or not target_alive()
    then
        want_since = 0
        return
    ...
    local hold = math.max(S.auto_delay, fire_gap_avg())
    ...
    if S.force then
        ...
        ocf, acf = compensate_force(ocf, acf, started)
        if fire_gun(gun, ocf, acf) then S.last_shot = now end
        return
    end
    ...
    if fire_gun(gun, cf, acf) then S.last_shot = now end
end
...
local function connect_gun_fired()
    ...
    _gun_fired_conn = m.GunFired.OnClientEvent:Connect(function(tool)
        ...
        local now = os.clock()
        if last_fire_stamp > 0 and want_since > 0
            and want_since <= last_fire_stamp
        then
...
function SilentAim.setForce(v)       S.force     = v   end
function SilentAim.setAuto(v)        S.auto_on   = v   end
function SilentAim.setDelay(v)       S.auto_delay = v  end
function SilentAim.setStandOff(v)    S.stand_off  = v  end
function SilentAim.setLeadScale(v)   P.lead_scale = v  end
function SilentAim.setLeadAdd(v)     P.lead_add   = v  end
...
function SilentAim.unload()
    ...
    restore_origin()
    uninstall_hooks()
    if _gun_fired_conn then
        pcall(function() _gun_fired_conn:Disconnect() end)
        _gun_fired_conn = nil
    end
    if _main_conn then
        pcall(function() _main_conn:Disconnect() end)
    end
end
...
SheriffTab = Window:Tab({ Title = "Sheriff", Icon = "crosshair" })
...
SheriffTab:Toggle({
    ...
    Value = false,
    Callback = function(v)
        if SilentAim and SilentAim.setForce then SilentAim.setForce(v) end
    end,
})
...
SheriffTab:Slider({
    ...
    Callback = function(v)
        if SilentAim and SilentAim.setStandOff then SilentAim.setStandOff(v) end
    end,
})
